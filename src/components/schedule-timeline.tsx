...
import { ChevronDown, Coffee, CircleDashed } from "lucide-react";
...
export function ScheduleTimeline({ schedule }: { schedule: Schedule }) {
  ...
  const totalMinutes = blocks.reduce((sum, b) => sum + minutesBetween(b.startTime, b.endTime), 0);
  const hours = Math.floor(totalMinutes / 60);
  const mins = totalMinutes % 60;
  const totalLabel = `${hours > 0 ? `${hours}h ` : ""}${mins}m`;

  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between text-xs">
        <span className="font-semibold uppercase tracking-wider text-muted-foreground">
          {taskCount} task{taskCount === 1 ? "" : "s"} · {totalLabel} scheduled
        </span>
      </div>
